...
def test_load_submissions(db_client: LeaderboardDBClient):
    existing_submissions = db_client.load_all_submissions(1)

    assert len(existing_submissions) == 0

def test_make_a_submission(db_client: LeaderboardDBClient):
    team_id = 1
    existing_submissions = db_client.load_all_submissions(team_id)
    assert len(existing_submissions) == 0

    submission = {
        "team_id": team_id,
        "git_commit_hash": "1234",
    }
    ...
    db_client.delete_submission(team_id, "1234")
    existing_submissions = db_client.load_all_submissions(team_id)
    assert len(existing_submissions) == 0
...
def test_leaderboard_db_updates(db_client):
    ...
    db_client.delete_submission(team_id, "098fffa0-bf29-45ef-81c8-b10d72aa62e2")
    existing_submissions = db_client.load_all_submissions(team_id)
    assert len(existing_submissions) == 0
