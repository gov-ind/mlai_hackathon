...
def test_eval_saves_to_db(docker_image, db_client, team_db_client, tm_client):
    start_time = datetime(2023, 4, 15, 0, 5, tzinfo=timezone.utc).timestamp()
    ...
    team_id = 1
    submissions = db_client.load_all_submissions(team_id)
    assert len(submissions) == 0

    current_working_directory = os.getcwd()
    data_dir = os.path.join(current_working_directory, "submission_backend", "output")
    full_eval(start_time, batch_end_time, data_dir, docker_image, team_id, "as5851234t32gre", task_id)

    task= tm.database.get_task(task_id)
    assert task['state_'] == 'success', task
    ...
    full_eval(start_time, batch_end_time, data_dir, docker_image, team_id, "as5851234t32gre", task2_id)

    task2 = tm.database.get_task(task2_id)
    assert task2['state_'] == 'success', task2
    ...
    db_client.delete_submission(team_id, "as5851234t32gre")
    submissions = db_client.load_all_submissions(team_id)
    assert len(submissions) == 0

def test_eval_task_raises_an_error_for_docker_error(broken_docker_image, db_client, team_db_client, tm_client):
    start_time = datetime(2023, 4, 15, 0, 5, tzinfo=timezone.utc).timestamp()
    end_time = datetime(2023, 4, 16, tzinfo=timezone.utc).timestamp() 
    ...
    current_working_directory = os.getcwd()
    data_dir = os.path.join(current_working_directory, "submission_backend", "output")
    full_eval(start_time, start_time, data_dir, broken_docker_image, team_id, "as5851234t32affddafse", task_id)

    loaded_task = tm.database.get_task(task_id)
    assert loaded_task['state_'] == 'error', loaded_task

    tm.database.delete_task(task_id)    

def test_eval_task_raises_an_error_for_system_error(docker_image, db_client, team_db_client, tm_client):
    start_time = datetime(2023, 4, 15, 0, 5, tzinfo=timezone.utc).timestamp()
    end_time = datetime(2023, 4, 16, tzinfo=timezone.utc).timestamp() 
    ...
    current_working_directory = os.getcwd()
    data_dir = os.path.join(current_working_directory, "submission_backend", "output")
    full_eval(start_time, start_time, data_dir, docker_image, "invalid team id", "as5851234t32affddafse", task_id)
